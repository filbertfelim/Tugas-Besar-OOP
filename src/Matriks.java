...
public class Matriks {
    ...
    public class SumbuY {
        ...
        public void setNilaiY(int y, int nilai) {
            if (y >= 0) {
                while (yPositif.size() <= y+1) yPositif.add(0);
                yPositif.add(y, nilai);
            } else {
                while (yNegatif.size() <= -(y+1) + 1) yNegatif.add(0);
                yNegatif.add(nilai);
            }
        }
    ...
    public void setNilai(int x, int y, int nilai) {
        ...
        if (x >= 0) {
            while (xPositif.size() <= x + 1) xPositif.add(sumbuY);
            xPositif.get(x).setNilaiY(y, nilai);
        } else {
            while (xNegatif.size() <= -(x+1) + 1) xNegatif.add(sumbuY);
